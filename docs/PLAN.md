...
**Status:** IN PROGRESS (started 2026-06-24). Workstreams: WS-A admin auth + crypto + audit (DONE); WS-B publish routine + Workers test pool (DONE); WS-C catalog data-layer (DONE); WS-D admin CRUD UI (fabric vertical slice + publish/unpublish DONE; widening to other entity types in progress); WS-E R2 image upload; WS-F storefront catalog read API (DONE); WS-G docs. Updated as each lands.
