...
**Status:** IN PROGRESS (started 2026-06-24). Workstreams: WS-A admin auth + crypto + audit (DONE); WS-B publish routine + Workers test pool (DONE); WS-C catalog data-layer (DONE); WS-D admin CRUD UI (DONE - all 8 entity types: garment types, base models + allow-lists, fabrics, option groups + values, upgrades, collections + members, attributes, supplier; per-item attribute-value assignment deferred to M6 discovery); WS-E R2 image upload (DONE; public storefront media serving deferred to go-live); WS-F storefront catalog read API (DONE); WS-G docs. Updated as each lands.
