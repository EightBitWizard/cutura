...
**Status:** DONE code-side (2026-06-24; commits 0dfb0ae..[WS-J], merged to main, gate-green). All workstreams: WS-A config/settings + feature-flag data layer (`packages/db/src/config`, `core/operationsSettings.ts`); WS-B capacity cap + vacation/pause + capacity-aware lead times (`core/capacity.ts`, `countOpenOrders`, storefront `server/ops.ts` gating `/api/cart` + `/api/checkout`, admin `/settings`); WS-C pipeline board + outlier surfacing that displays (`core/checkOutliers`, `db/ops/board.ts`, admin `/orders/board` + detail warning - the FR-1020 regression); WS-D fit-review queue + audited pre-release correction (`db/ops/fitReviewQueue.ts` + `correction.ts`, admin `/fit-reviews`); WS-E customer management + notes/tags (migration 0003, `db/ops/customers.ts` with audited measurement views, admin `/customers`); WS-F supplier management (env DB, single-default routing) + shipping config (`db/suppliers`, `db/shipping`, admin `/suppliers` + `/shipping`); WS-G KPI dashboard + cost capture + margin (`core/kpiMath.ts`, `db/kpi`, admin `/dashboard`); WS-H CSV export (no PII) + admin notifications (`db/ops/export.ts`, `notifyAdmin`, webhook new-order); WS-I shared admin nav shell + audit-log view (`/audit`); WS-J ADR 0005 + docs. Tests: core 104, db 19 node + 57 workers, admin 10, storefront 5. CI quality/guards/secret-scan green; deploy-staging red pending provisioning. OUT (deferred): RBAC roles (FR-1040), supplier-performance tracking (FR-10B0), paid express shipping (FR-291). The LIVE half (real admin emails, the end-to-end Staging operational loop) awaits Resend + Cloudflare provisioning. Follow-ups: QC-due notification trigger (helper exists, no arrived_ch admin action yet); deep checkout wiring of shipping methods; serving fit-review photos.
...
**Status:** DONE code-side (2026-06-24; commits 897fc8e..[WS-L], merged to main, gate-green). 12 workstreams: WS-A public media serving (`core/media.ts` allow-list, storefront `/api/media/[id]` safe serve, `primaryMediaForEntities`, images on cards/PDP) + R2-object-copy-on-publish noted as a go-live follow-up; WS-B `core/i18nDetect.ts` pickLocale + middleware browser detection + cookie + LanguageSwitcher; WS-C `core/seo.ts` buildAlternates + `core/format.ts` + generateMetadata (home/PDP/collection/content/discovery) + `app/sitemap.ts` + `robots`; WS-D contentPage publish resolver + read helpers + admin `/content` + storefront `/content/[slug]` & `/legal/[slug]` (DB-backed, legal placeholders removed) + fibre composition on PDP (FR-321); WS-E `listPublishedModelsFiltered`+`listAttributeFacets` + `/discover`; WS-F `getPublishedCollection` + `/collections/[handle]` + `/occasions/[value]` + `/search`; WS-G notify-me (`db/notify`, `/api/notify-me` rate-limited, PDP form); WS-H attribute-value assignment helpers + admin UI on base-model edit + collection banner upload; WS-I cross-sell (migration 0004 `crossSellRule` + publish resolver + admin `/cross-sell` + PDP suggestions); WS-J localized parcel card (`core/parcelCard.ts` + `db/orders/parcelCardPdf.ts`) + admin packaging step `/api/orders/[id]/parcel-card`; WS-K designed 404 + fit-guide link + seeded content pages; WS-L ADR 0006 + docs. Tests: core 116, db 21 node + 66 workers, admin 10, storefront 5. CI quality/guards/secret-scan green; deploy-staging red pending provisioning. OUT (deferred): Spanish (FR-1280), config sharing (FR-3B0), recently-viewed (FR-3C0), editable nav (FR-372), lookbook (FR-391), rule-based collections (FR-2D1). The LIVE half (real images in the env bucket, end-to-end four-language verification + SEO/hreflang validation, translator + lawyer review of copy/legal) awaits provisioning. Follow-ups: copy R2 objects to the env bucket on publish; fabric attribute-assignment UI; search/facet indexing at scale.
...
**Status:** DONE code-side (2026-06-24; commits 25b79c3..[WS-J], merged to main, gate-green). 10 workstreams: WS-A consent banner (`core/consent.ts` single `hasAnalyticsConsent` gate, opt-in, no analytics integrated); WS-B legal pages (terms/privacy/imprint/shipping/fit-guarantee seeded honest/non-fabricated, FR-13B0 published policy) + footer + imprint placeholders; WS-C contact form + help (`contactMessage` table, `db/contact` submit+email, `/contact` + rate-limited route); WS-D maintenance mode (`operationsSettings.maintenance` + layout gate; admin/`/api` stay reachable) + a contact-email HTML-escape security fix; WS-E bot/abuse (`core/turnstile.ts` no-op-without-secret + rate limits on checkout/contact); WS-F SEO (`core/productJsonLd.ts` on PDP with `<` escaped + `redirect` table + middleware lookup + admin `/redirects`); WS-G sewn-in label spec (`SewnInLabel` in supplier spec/PDF + `getFabricSewnInLabel`); WS-H perf/a11y CI (jsx-a11y already enforced via next/core-web-vitals; `e2e/a11y.spec.ts` + gated `lighthouse.yml`, live-deferred); WS-I backup/restore drill (`infra/restore-drill.md`+`.sh`) + billing-cap doc; WS-J `docs/COMPLIANCE.md` (FR/NFR evidence map + data inventory + retention) + ADR 0007 + docs. Migration 0005 (contact_message + redirect). Tests: core 126, db 22 node + 70 workers, admin 10, storefront 5. CI quality/guards/secret-scan green; deploy-staging red pending provisioning. OUT: VAT invoice (FR-1381), AI guardrails (FR-13C0), import VAT (FR-13D0). FOUNDER/EXTERNAL before launch (tracked in COMPLIANCE.md): real imprint company data + lawyer sign-off of legal pages; textile-labeling + import-VAT advisor; live Lighthouse + a11y; the executed restore drill + billing cap; D1/R2 region (residency); Turnstile keys.
...
**Status:** DONE code-side (2026-06-24; commits 17e17ff..[WS-G], merged to main, gate-green). 7 workstreams: WS-A garment-type-aware measurement flow (`core/garmentFields.ts`; MeasurementFlow + `/api/measurement` dispatch estimate/checkOutliers by type; measurements stored per garment type - guest KV keyed, customers one profile per type via `getProfileIdForGarmentType`; checkout resolves per line; cart/checkout show per-type status) - removes the last shirt-coupling so FR-104 holds; WS-B trouser as data (seed garment type + schema + Chino model + fabrics + option + allow-lists + cross-sell; no code beyond WS-A); WS-C mixed shirt+trouser multi-item Workers test (FR-850/851/852); WS-D recommendation seam (`core/recommender.ts` mirrors the estimator) + content-based/curated baseline (`db/recommend`); WS-E consent-gated signal capture (`recommendation_signal`, migration 0006; `db/signals`; `/api/signal`; PDP beacon) + deletion (FR-1142); WS-F `RecommendedSection` on home/PDP/cart/account/order/track; WS-G ADR 0008 + docs. Tests: core 135, db 24 node + 75 workers, admin 10, storefront 5. CI quality/guards/secret-scan green; deploy-staging red pending provisioning. OUT (Future, behind the seams): ML personalization (FR-1150), AI try-on (FR-1160/61), prompt-to-bespoke (FR-1170-72), bundle pricing (FR-1401, Post-launch). Deferred live (founder): a real trouser order + a real two-item order on Staging; recommendations on the deployed surfaces; reviewed trouser translations.
...
**Status:** the CODEABLE launch-hardening is DONE (2026-06-24; commits 3ea6b78..[WS-F], merged to main, gate-green). The release machinery already existed (`release-production.yml` gated promotion + production smoke; both wrangler envs; founder runbooks). This batch closed the deferred code follow-ups: WS-A scheduled reconcile (`.github/workflows/reconcile.yml` -> the bearer-guarded endpoint); WS-B GDPR webhooks (`customers/redact` -> `redactCustomerByEmail`; data_request + shop/redact audited for the founder); WS-C `middleware.ts` -> `proxy.ts` (Next 16); WS-D media-object copy on publish (`copyPublishedMedia`, injected buckets, self-healing); WS-E Shopify refund execution from the fit-review decision (`executeOrderRefund`, injected + audited, live-gated); WS-F ADR 0009 + docs. No schema change. Tests: core 135, db 24 node + 80 workers, admin 10, storefront 5. **Everything else in M9 is FOUNDER/LIVE, not code:** run `infra/setup-runbook.md` (Cloudflare provisioning + worker secrets + GitHub `CLOUDFLARE_API_TOKEN`/`ACCOUNT_ID` + `STAGING_URL`/`PRODUCTION_URL` vars + `RECONCILE_BEARER` + the `production` GitHub environment); Shopify custom app + TWINT/wallets + tax-inclusive VAT + a real paid order; lawyer sign-off + real imprint data; textile-labeling + import-VAT advisors; SPF/DKIM/DMARC; DNS cutover; the executed restore drill + billing cap; D1/R2 region (residency); optional admin `SHOPIFY_*` (refunds) + `TURNSTILE_SECRET`. **Acceptance gate (founder-run):** a real end-to-end paid order completes on Production, every Appendix C item is confirmed, and the founder can independently execute the code release, catalog release, rollback, restore, and pause runbooks.
