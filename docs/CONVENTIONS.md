...
- The request gate stays `middleware.ts` (both apps; exports `middleware`). Next 16
  deprecates this for `proxy.ts`, but `@opennextjs/cloudflare` ^1.19 rejects the proxy
  convention as "Node.js middleware" at `cf:build`. Revisit the rename once OpenNext
  supports it. NOTE: `next build` accepts proxy.ts but `cf:build` does not, so always
  run `pnpm --filter @cutura/storefront cf:build` before trusting a middleware change.
